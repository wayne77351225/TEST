    ...
    class Hid : object
    {
        ...
        public HID_RETURN OpenDevice(UInt16 vID, UInt16 pID, string serial)
        {
            if (deviceOpened == false)
            {
                ...
                getHIDRegistryPath(ref deviceList);
        ...
        public void getHIDRegistryPath(ref List<string> deviceList)
        {
            //4d1e55b2-f16f-11cf-88cb-001111000030=>hid device的guid
            RegistryKey rkUsbPrint = Registry.LocalMachine.OpenSubKey("SYSTEM\\CurrentControlSet\\Control\\DeviceClasses\\{4d1e55b2-f16f-11cf-88cb-001111000030}");
            string path = null;
            if (rkUsbPrint != null)
            {
